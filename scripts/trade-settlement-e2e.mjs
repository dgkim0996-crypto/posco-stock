...
try{
  ...
  assert(Number(sellOrder.fee_amount)===8.25&&Number(sellOrder.tax_amount)===110,"매도 수수료 또는 거래세 불일치");
  assert(Number(sellOrder.transaction_tax_amount)===27.5&&Number(sellOrder.agricultural_tax_amount)===82.5,"KOSPI 세금 구성 불일치");
  assert(Number(sellAccount.krw_balance)===954866.75,"매도 후 잔액 불일치");

  await client.query("select public.place_spot_order($1,'009520','stocks','BUY','MARKET',1,null,10000)",[accountId]);
  const kosdaq=(await client.query("select public.place_spot_order($1,'009520','stocks','SELL','MARKET',1,null,10000) result",[accountId])).rows[0].result;
  const kosdaqOrder=(await client.query("select * from public.orders where id=$1",[kosdaq.order.id])).rows[0];
  assert(Number(kosdaqOrder.transaction_tax_amount)===20&&Number(kosdaqOrder.agricultural_tax_amount)===0,"KOSDAQ 세금 구성 불일치");
  ...
  assert(settlements.length===6,"결제 예정내역 건수 불일치");
  assert(Number(settlements[0].net_amount)===-100015,"매수 순결제금액 불일치");
  assert(Number(settlements[1].net_amount)===54881.75,"매도 순결제금액 불일치");
  ...
  assert(counts.TRADE_FEE===6&&counts.TRADE_TAX===2,"수수료·세금 원장 건수 불일치");
  ...
  console.log(JSON.stringify({result:"PASS",buy:{gross:100000,fee:15,tax:0,net:-100015},kospiSell:{gross:55000,fee:8.25,transactionTax:27.5,agriculturalTax:82.5,net:54881.75},kosdaqSell:{transactionTax:20,agriculturalTax:0},etfSellTax:0,weekendD2:"2026-09-15",settlements:settlements.length,cashLedgerEntries:cashLedgerCount,accountLedgerEntries:accountLedgerCount,rolledBack:true},null,2));
